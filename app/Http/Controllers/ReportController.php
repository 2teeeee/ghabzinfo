...
{
    public function electricityDashboard(Request $request): View
    {
    ...
    public function gasDashboard(Request $request): View
    {
        // گرفتن تاریخ آخرین دوره
        $lastPeriod = GasBillPeriod::orderByDesc('current_date')->first();
        $lastDate = $lastPeriod ? Carbon::parse($lastPeriod->current_date) : now();
        ...
        // 2️⃣ تعداد کنتورها
        // -------------------------------
        $counterCount = GasAccount::whereHas('periods', function($q) use ($start, $end) {
            $q->whereBetween('current_date', [$start, $end]);
        })->count();
        ...
        // 3️⃣ مبلغ برق تجمیعی و مصرف
        // -------------------------------
        $aggregates = GasBillPeriod::whereBetween('current_date', [$start, $end])
            ->select(
                DB::raw('SUM(amount) as total_amount'),
                DB::raw('SUM(CAST((current_counter_digit - previous_counter_digit) AS DECIMAL)) as total_consumption')
            )
            ->first();
        ...
        for ($i = 0; $i < 3; $i++) {
            $y = $currentYear - $i;
            $sum = GasBillPeriod::whereYear('current_date', $y)->sum('amount');
            $yearlyComparison[] = [
                'year' => $y,
                'amount' => $sum,
            ];
        ...
        // 5️⃣ کاهش مصرف ماه آخر نسبت به ماه مشابه سال قبل
        // -------------------------------
        $currentMonthAmount = GasBillPeriod::whereBetween('current_date', [$start, $end])
            ->sum(DB::raw('CAST((current_counter_digit - previous_counter_digit) AS DECIMAL)'));

        $lastYearStart = (clone $start)->subYear();
        $lastYearEnd   = (clone $end)->subYear();

        $lastYearMonthAmount = GasBillPeriod::whereBetween('current_date', [$lastYearStart, $lastYearEnd])
            ->sum(DB::raw('CAST((current_counter_digit - previous_counter_digit) AS DECIMAL)'));
        ...
        for ($i = 2; $i >= 0; $i--) {
            ...
            $seasonData->push([
                'month' => Jalalian::fromCarbon($tempStart)->format('Y/m'),
                'value' => GasBillPeriod::whereBetween('current_date', [$tempStart, $tempEnd])
                    ->sum(DB::raw('CAST((current_counter_digit - previous_counter_digit) AS DECIMAL)')),
            ]);
        }
        ...
        // نمایش view
        // -------------------------------
        return view('admin.reports.gas_dashboard', compact(
            'month',
            'year',
            'counterCount',
            'totalAmount',
    ...
    public function waterDashboard(Request $request): View
    {
        // گرفتن تاریخ آخرین دوره
        $lastPeriod = WaterBillPeriod::orderByDesc('current_date')->first();
        $lastDate = $lastPeriod ? Carbon::parse($lastPeriod->current_date) : now();
        ...
        // 2️⃣ تعداد کنتورها
        // -------------------------------
        $counterCount = WaterAccount::whereHas('periods', function($q) use ($start, $end) {
            $q->whereBetween('current_date', [$start, $end]);
        })->count();
        ...
        for ($i = 0; $i < 3; $i++) {
            $y = $currentYear - $i;
            $sum = WaterBillPeriod::whereYear('current_date', $y)->sum('amount');
            $yearlyComparison[] = [
                'year' => $y,
                'amount' => $sum,
            ];
        ...
        // 5️⃣ کاهش مصرف ماه آخر نسبت به ماه مشابه سال قبل
        // -------------------------------
        $currentMonthAmount = WaterBillPeriod::whereBetween('current_date', [$start, $end])
            ->sum(DB::raw('CAST(amount AS DECIMAL)'));

        $lastYearStart = (clone $start)->subYear();
        $lastYearEnd   = (clone $end)->subYear();

        $lastYearMonthAmount = WaterBillPeriod::whereBetween('current_date', [$lastYearStart, $lastYearEnd])
            ->sum(DB::raw('CAST(amount AS DECIMAL)'));
        ...
        for ($i = 2; $i >= 0; $i--) {
            ...
            $seasonData->push([
                'month' => Jalalian::fromCarbon($tempStart)->format('Y/m'),
                'value' => WaterBillPeriod::whereBetween('current_date', [$tempStart, $tempEnd])
                    ->sum(DB::raw('CAST(amount AS DECIMAL)')),
            ]);
        }
        ...
        // نمایش view
        // -------------------------------
        return view('admin.reports.water_dashboard', compact(
            'month',
            'year',
            'counterCount',
            'totalAmount',
