    ...
    <div class="container py-4">
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                ...
                <p class="card-text mb-1"><strong>نوع تعرفه:</strong> {{ $account->tariff_type }}</p>
                <p class="card-text mb-1"><strong>نوع مشتری:</strong> {{ $account->customer_type }}</p>
                ...
                <div class="accordion-item mb-2 shadow-sm">
                    <h2 class="accordion-header" id="heading-{{ $period->id }}">
                        <button class="accordion-button {{ $index != 0 ? 'collapsed' : '' }}"
                            ...
                            دوره {{ $period->cycle ?? '-' }} / {{ $period->sale_year ?? '-' }} - مبلغ: {{ number_format($period->amount) }} تومان
                        </button>
                    </h2>
                    <div id="collapse-{{ $period->id }}"
                        ...
                        <div class="accordion-body">
                            ...
                            <p class="mb-1"><strong>متوسط مصرف:</strong> {{ $period->average_consumption ?? '-' }}</p>
                            <p class="mb-1"><strong>بیمه:</strong> {{ $period->insurance_amount ?? '-' }}</p>
                            <p class="mb-1"><strong>مالیات:</strong> {{ $period->tax_amount ?? '-' }}</p>
                            <p class="mb-1"><strong>عوارض:</strong> {{ $period->paytoll_amount ?? '-' }}</p>
                            <p class="mb-1"><strong>عوارض برق:</strong> {{ $period->power_paytoll_amount ?? '-' }}</p>
                            <p class="mb-1"><strong>تعداد روزها:</strong> {{ $period->total_days ?? '-' }}</p>
