<x-admin-layout title="لیست قبوض گاز" header="لیست قبوض گاز">
    <div class="container py-4">
        <h4 class="mb-3">لیست قبوض گاز</h4>
        ...
        <form method="GET" class="mb-4">
            <div class="input-group">
                ...
                <a href="{{route('admin.gas_bills.create')}}" class="btn btn-info ms-2">قبض جدید</a>
            </div>
        </form>
            ...
            <table class="table table-hover align-middle">
                ...
                @forelse($accounts as $account)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin.gas_bills.show', $account->id) }}" class="btn btn-sm btn-info">جزئیات</a>
                            <form action="{{ route('admin.gas_bills.refresh', $account->id) }}" method="POST" class="d-inline">
                                @csrf
                                <button type="submit" class="btn btn-sm btn-warning">استعلام جدید</button>
                            </form>
                            <form action="{{ route('admin.gas_bills.destroy', $bill->id) }}" method="POST" onsubmit="return confirm('آیا مطمئنید که می‌خواهید این قبض را حذف کنید؟')">
                                @csrf
                                @method('DELETE')
