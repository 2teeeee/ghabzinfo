<x-admin-layout title="ویرایش مرکز" header="ویرایش مرکز">
    <div class="container-fluid py-4">
        ...
        <div class="card border-0 shadow-lg rounded-0">
            <div class="card-body px-4 py-5">
                <form id="editCenterForm" action="{{ route('admin.centers.update', $center->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="city_id" name="city_id" class="form-select" {{ $isCity || $isOrgan ? 'disabled' : '' }} required>
                                ...
                                @foreach($cities as $city)
                                    <option value="{{ $city->id }}" @selected(old('city_id', $center->unit->organ->city_id) == $city->id)>{{ $city->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-4 position-relative">
                            ...
                            <select id="organ_id" name="organ_id" class="form-select" {{ $isOrgan ? 'disabled' : '' }} required>
                                <option value="{{ $center->unit->organ_id }}">{{ $center->unit->organ->name }}</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4 position-relative">
                            ...
                            <select id="unit_id" name="unit_id" class="form-select" required>
                                <option value="{{ $center->unit_id }}">{{ $center->unit->name }}</option>
                            </select>
                        </div>
